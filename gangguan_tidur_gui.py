...
# Inisialisasi prolog
prolog = Prolog()
prolog.consult("gangguan_tidur_gui.pl")  # Nama file diubah
penyakit = list()
gejala = dict()
index_penyakit = 0
index_gejala = 0
...
root = tk.Tk()
root.title("Sistem Pakar Diagnosis Gangguan Tidur")  # Judul diubah
...
ttk.Label(mainframe, text="Aplikasi Diagnosa Gangguan Tidur",  # Judul diubah
         font=("Arial", 16)).grid(column=0, row=0, columnspan=3)
ttk.Label(mainframe, text="Kolom Pertanyaan:").grid(column=0, row=1)
kotak_pertanyaan = tk.Text(mainframe, height=4, width=50,  # Lebar diperbesar
                         state=tk.DISABLED)
